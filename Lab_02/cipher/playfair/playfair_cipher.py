class PlayFairCipher:
    ...
    def playfair_decrypt(self, cipher_text, matrix):
        ...
        # Loại bỏ ký tự 'X' nếu là ký tự cuối cùng và là ký tự đệm
        final_text = ""
        for i in range(0, len(decrypted_text) - 2, 2):
            if decrypted_text[i] == decrypted_text[i+2]:
                final_text += decrypted_text[i]
            else:
                final_text += decrypted_text[i] + decrypted_text[i+1]

        if decrypted_text[-1] == "X":
            final_text += decrypted_text[-2]
        else:
            final_text += decrypted_text[-2] + decrypted_text[-1]
